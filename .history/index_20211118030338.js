...
bookapi.get("/authors/:author", (req,res) => {
    const auth = database.authors.filter((author) => author.name === req.params.author)
    const getSpecificBooks = database.books.filter((book) =>
    book.authorid.includes(req.params.aid)
  );

  if (getSpecificBooks.length === 0) {
    return res.json({
      error: `No book found for the author ID of ${req.params.aid}`,
    });
  }
