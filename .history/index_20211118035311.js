...
bookapi.get("/books/:isbn", (req, res) => {
...
bookapi.get("/books/:category", (req, res) => {
...
bookapi.get("/books/:author", (req, res) => {
  const getSpecificBooks = database.books.filter((book) =>
    book.authorid.includes(
      (authid) => authid === database.authors.filter(req.params.author)[0].id
    )
  );

  if (getSpecificBooks.length === 0) {
    return res.json({
      error: `No book found for the author ${req.params.category}`,
    });
  }
